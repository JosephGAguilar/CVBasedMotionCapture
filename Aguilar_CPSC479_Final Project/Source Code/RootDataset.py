...
class DatasetLoader(Dataset):
    ...
    def __getitem__(self, index):
        ...
        img_patch, trans = r_generate_patch_image(cvimg, bbox, do_flip, rot)
        ...
        root_vis *= (
                        (root_img[0] >= 0) & \
                        (root_img[0] < r_cfg.input_shape[1]) & \
                        (root_img[1] >= 0) & \
                        (root_img[1] < r_cfg.input_shape[0])
                        )
        
        # change coordinates to output space
        root_img[0] = root_img[0] / r_cfg.input_shape[1] * r_cfg.output_shape[1]
        root_img[1] = root_img[1] / r_cfg.input_shape[0] * r_cfg.output_shape[0]
        
        if self.is_train:
            img_patch = self.transform(img_patch)
            k_value = np.array([math.sqrt(r_cfg.bbox_real[0]*r_cfg.bbox_real[1]*f[0]*f[1]/(area))]).astype(np.float32)
...
def r_generate_patch_image(cvimg, bbox, do_flip, rot):
    img = cvimg.copy()
    ...
    trans = gen_trans_from_patch_cv(bb_c_x, bb_c_y, bb_width, bb_height, r_cfg.input_shape[1], r_cfg.input_shape[0], rot, inv=False)
    img_patch = cv2.warpAffine(img, trans, (int(r_cfg.input_shape[1]), int(r_cfg.input_shape[0])), flags=cv2.INTER_LINEAR)
