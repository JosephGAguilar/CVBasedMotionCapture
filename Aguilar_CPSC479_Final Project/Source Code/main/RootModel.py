...
class RootNet(nn.Module):
    ...
    def forward(self, x, k_value):
        ...
        xy = self.xy_layer(xy)
        xy = xy.view(-1,1,r_cfg.output_shape[0]*r_cfg.output_shape[1])
        xy = F.softmax(xy,2)
        xy = xy.view(-1,1,r_cfg.output_shape[0],r_cfg.output_shape[1])
        ...
        if r_cfg.gpu_ids != 'n':
            coord_x = hm_x * torch.arange(r_cfg.output_shape[1]).float().cuda()
            coord_y = hm_y * torch.arange(r_cfg.output_shape[0]).float().cuda()
        else:
            coord_x = hm_x * torch.arange(r_cfg.output_shape[1]).float()
            coord_y = hm_y * torch.arange(r_cfg.output_shape[0]).float()
...
def get_root_net(r_cfg, is_train):
    
    backbone = ResNetBackbone(r_cfg.resnet_type)
    root_net = RootNet()
    if is_train:
