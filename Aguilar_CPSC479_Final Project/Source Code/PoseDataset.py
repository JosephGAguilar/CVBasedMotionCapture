...
from utils.PoseVis import p_vis_keypoints, p_vis_3d_skeleton
from utils.PoseUtils import fliplr_joints, transform_joint_to_other_db
...
class DatasetLoader(Dataset):
    ...
    def __getitem__(self, index):
        ...
        img_patch, trans = p_generate_patch_image(cvimg, bbox, do_flip, scale, rot, do_occlusion)
        ...
        for i in range(len(joint_img)):
            ...
            joint_img[i, 2] /= (p_cfg.bbox_3d_shape[0]/2.) # expect depth lies in -bbox_3d_shape[0]/2 ~ bbox_3d_shape[0]/2 -> -1.0 ~ 1.0
            joint_img[i, 2] = (joint_img[i,2] + 1.0)/2. # 0~1 normalize
            joint_vis[i] *= (
                            (joint_img[i,0] >= 0) & \
                            (joint_img[i,0] < p_cfg.input_shape[1]) & \
                            (joint_img[i,1] >= 0) & \
                            (joint_img[i,1] < p_cfg.input_shape[0]) & \
                            (joint_img[i,2] >= 0) & \
                            (joint_img[i,2] < 1)
                            )
        ...
        joint_img[:, 0] = joint_img[:, 0] / p_cfg.input_shape[1] * p_cfg.output_shape[1]
        joint_img[:, 1] = joint_img[:, 1] / p_cfg.input_shape[0] * p_cfg.output_shape[0]
        joint_img[:, 2] = joint_img[:, 2] * p_cfg.depth_dim
...
def p_generate_patch_image(cvimg, bbox, do_flip, scale, rot, do_occlusion):
    img = cvimg.copy()
    ...
    trans = gen_trans_from_patch_cv(bb_c_x, bb_c_y, bb_width, bb_height, p_cfg.input_shape[1], p_cfg.input_shape[0], scale, rot, inv=False)
    img_patch = cv2.warpAffine(img, trans, (int(p_cfg.input_shape[1]), int(p_cfg.input_shape[0])), flags=cv2.INTER_LINEAR)
